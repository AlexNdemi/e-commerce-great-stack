...
import { Image } from '../ui/ImageWithSkeleton.tsx';
    ...
    <div className={`border-t-2 pt-10 ${theme === "dark" ? "border-t-[rgba(255,255,255,0.2)]" : "border-t-gray-300"}`}>
      ...
      <div className="flex gap-10 flex-col sm:flex-row"> 
        ...
        <div className="flex-1 flex flex-col sm:flex-row gap-3"> 
          {/* Thumbnail images */} 
          <div className="flex sm:flex-col overflow-x-auto sm:overflow-y-auto justify-between sm:justify-start sm:w-[18.7%] gap-2 order-1 sm:order-0 w-full"> 
            {productData.image.map((item, index) => ( 
              <Image
                key={index}
                src={item}
                alt={`Product image ${index + 1}`}
                className='w-[24%] sm:w-full flex-shrink-0'
                imageClassName='cursor-pointer hover:opacity-80 transition-opacity'
        ...
        <div className="flex-1"> 
          ...
          <p className={`mt-5 ${theme === 'dark' ? 'text-[#aaa]' : ''} md:max-w-4/5`}>
            ...
            <div className='flex flex-col gap-4'>
              <div className={`flex gap-4 items-center max-w-60`}>
                ...
                <button 
                  ...
                  className={`w-10 h-10 rounded-[7px] flex items-center justify-center transition-all duration-200 transform ${
                    theme === 'dark' 
                      ? 'bg-white text-black hover:bg-gray-200 active:bg-gray-300'
                      : 'bg-black text-white hover:bg-gray-800 active:bg-gray-700'
                  } shadow-lg`}
          ...
          <hr className={`mt-8 ${theme === "dark" ? "bg-[rgba(255,255,255,0.2)]" : "bg-gray-300"} border-0 h-[1px]`} />
          <div className={`text-sm mt-5 flex flex-col gap-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
